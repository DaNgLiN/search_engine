...
def make_index(dir):
    ...
    browseing_dir = dir / ".browseing"
    browseing_dir.mkdir(exist_ok=True)
    ...
    with (browseing_dir / "documents.csv").open('w', encoding="utf-8") as f:
        out = csv.writer(f)
        for doc in documents:
            out.writerow(doc)

    # Save the index itself.
    with (browseing_dir / "index.dat").open('wb') as f:
        start = 0
    ...
    with (browseing_dir / "terms.csv").open('w',  encoding="utf-8") as f:
        out = csv.writer(f)
...
class Index:
    """Object for querying a .browseing index."""

    def __init__(self, dir):
        """Create an Index that reads `$DIR/.browseing`."""
        dir = pathlib.Path(dir)
        browseing_dir = dir / ".browseing"
        self.dir = dir
        self.index_file = browseing_dir / "index.dat"

        self.documents = []
        for [line, max_tf] in csv.reader((browseing_dir / "documents.csv").open('r')):
            self.documents.append(Document(pathlib.Path(line), int(max_tf)))

        self.terms = {}
        for word, start, length in csv.reader((browseing_dir / "terms.csv").open('r')):
